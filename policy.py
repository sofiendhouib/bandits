...
#from celer import Lasso as LassoCeler
...
def ucb1(cumul_rewards, n_pulls, t, a):
    if callable(a): a = a(t)
...
class PolicyContextual():
    ...
    def __init__(self, l_reg= 1, delta= 0.1):
        self.l_reg = l_reg
        self.delta = delta
...
class LinUCB(PolicyContextual):
    ...
    def play_arm(self, cxt_mat, t):
        dim = cxt_mat.shape[1]
        sqrtbeta = sqrt(self.l_reg) + sqrt(2*log(1/self.delta)+dim*log(1+t/(dim*self.l_reg)))
        radius = sqrtbeta * np.sqrt(np.einsum("ij,jkl,ik->il", cxt_mat, self.invV, cxt_mat))
        ucb = cxt_mat @ np.einsum("ijk,jk->ik", self.invV, self.b) + radius
    ...
    def update(self, x, y, t):
        invV_x = np.einsum("ijk, jk -> ik", self.invV , x)
        self.invV -= np.einsum("ik,jk -> ijk", invV_x, invV_x)/(1 + np.einsum("ik,ik->k", x, invV_x)[None,None,:])
        self.b += y * x
        return self

# TODO code a version that tracks the data matrix when dim is very high
class LALasso():
    ...
    def update(self, x, y, t):
        ...
        l_t = self.l_reg*sqrt((4*log(t) + 2*log(len(self.b)))/t)
...
class GraphLinUCB():
    ...
    def initialize(self, bandit):
        ...
        self.aL = self.a * (0.5*(bandit.L + bandit.L.T) + 0.01*np.eye(bandit.n_users)) 
        # self.aL = self.a * (bandit.L + 0.01*np.eye(bandit.n_users))
...
def solve_lasso(V, b, l, theta, lr, t):
    "We use the FISTA algorithm"    
    #err = np.inf
    #value = value_lasso(V, b, theta, l ,t)
    v = theta.copy()
    lamb= 0
    for _ in range(1000):
        grad = 0.5/t*(np.einsum("ijk, jk->ik", V, theta)- b)# the gradient of the L1 regularization is not taken
        v = - lr*grad
        theta_new = v - np.clip(v, -lr*l, lr*l) # proximal shrinkage operator
        lamb = 0.5*(1+sqrt(1 + 4*lamb**2))
        gamma = 2*(1-lamb)/(1+sqrt(1+4*lamb**2))
        v = (1-gamma) * theta_new + gamma * theta
